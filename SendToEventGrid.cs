...
namespace TollBooth
{
    public class SendToEventGrid
    {
        ...
        public async Task SendLicensePlateData(LicensePlateData data)
        {
            ...
            {
                //  Modify send method to include the proper eventType name value for saving plate data.
                await Send("savePlateData", "TollBooth/CustomerService", data);
            }
            else
            {
                // Modify send method to include the proper eventType name value for queuing plate for manual review.
